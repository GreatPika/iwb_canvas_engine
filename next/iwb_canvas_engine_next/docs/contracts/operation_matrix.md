...
| CanvasEdit.removeElement | registry, layer membership, selection when removed id was selected | document, structural, bounds, visual, projection, selection if selected | remove id | evict | main | none |
| command removeElement | registry, layer membership, selection when removed id was selected | document, structural, bounds, visual, projection, selection if selected | remove id | evict | main | deleteElements if removed |
...
| eraser commit | removed elements | document, structural, bounds, visual, projection, selection if erased ids intersect selection | remove ids | evict | main + overlay cleanup | erase if removed |
