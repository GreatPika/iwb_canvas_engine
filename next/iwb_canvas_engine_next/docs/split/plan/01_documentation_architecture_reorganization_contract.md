...
- Keep `docs/split/plan/` for Change Contracts. It is not a target architecture, contract, verification, or planning source folder.
...
- `docs/split/plan/01_documentation_architecture_reorganization_contract.md`
...
- `AGENTS.md`
- `tool/diagrams/c4_context.mmd`
- `tool/diagrams/c4_container.mmd`
- `tool/diagrams/c4_component_runtime.mmd`
...
- Creation or update of exactly these generated Mermaid diagram files under `next/iwb_canvas_engine_next/tool/diagrams/`:
  - `c4_context.mmd`
  - `c4_container.mmd`
...
- Create `architecture/`, `contracts/`, `verification/`, and `planning/`; keep `plan/` for Change Contracts.
...
- The navigation command must validate the architecture README role routing for `architecture/`, `contracts/`, `verification/`, `planning/`, `plan/`, and `donors/`.
...
- Keep this change contract in `docs/split/plan/01_documentation_architecture_reorganization_contract.md`.
...
- The generator writes exactly these Mermaid files under `tool/diagrams/`: `c4_context.mmd`, `c4_container.mmd`, and `c4_component_runtime.mmd`.
- `generate_architecture_diagrams.dart --check` regenerates the expected Mermaid content in memory and fails if any on-disk `tool/diagrams/*.mmd` output differs or is missing.
...
- Run `dart run docs/split/tool/check_split_navigation.dart`.
- Run `test -f tool/diagrams/c4_context.mmd`.
- Run `test -f tool/diagrams/c4_container.mmd`.
- Run `test -f tool/diagrams/c4_component_runtime.mmd`.
...
- Change contracts for this reorganization live under `docs/split/plan/`.
