...
| CanvasTextEditingPort | single runtime-owned active text edit session, read-only admission, transient new-text seed/destination guard, captured empty-text policy, live text geometry/style projection, permanently latched stale-draft retention, and typed commit/cancel terminal lifecycle | own Flutter IME/editor widgets, mutate document visibility to hide text, duplicate the interaction guard comparison, or replace context-action ownership |
