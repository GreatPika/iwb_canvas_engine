...
| `generic-invalidation-retirement` | Design D11 replaces image-named invalidation with resource-generic target/all release, preserves accepted publication after later notification failure, and retires private mirrors without a permanent token scanner; Unit 1 first establishes that seam and its behavioral evidence, then closes its source owners and graph. | Boundaries; Unit 1 | `generic-resource-release` |
...
- `lib/src/resources/surface_resource_session.dart:210` / Unit 1 commit `75a4c410` first adds generic target/all release, resolver replacement, reset, and drop that remove session borrows and invoke retained-output callbacks -> behavioral evidence precedes release-specific owner closure.
- `lib/src/surface/layer_frame_output_cache.dart:80` / Unit 1 commit `75a4c410` first removes matching retained main-output bindings while preserving unrelated bindings and overlay -> Unit 1 semantic diagrams and graph record this identity-aware surface ownership.
- `docs/contracts/resources.md:61` / pre-Unit 1 commit `92fad1b5` names `ResourceSessionInvalidationSink` and session-cache-only invalidation -> Unit 1 closes resource, operation, runtime-ownership, diagram, and graph truth before later units consume the lifecycle.
- `docs/architecture/architecture_graph.yaml:614` / pre-Unit 1 commit `92fad1b5` lists `ResourceSessionInvalidationSink` as the runtime-to-session actual composition field -> Unit 1 updates that existing edge and its source documents together rather than adding a graph owner.
...
### [ ] Unit 1: Establish generic non-fallible resource release and close its semantics and graph ownership

Owner: Internal resource release contract, runtime resource delivery, surface resource session, retained frame-output lifecycle, release-specific resource and operation contracts, runtime-ownership documentation, semantic diagrams, the existing runtime-to-session graph edge, and their generated projections.
Boundary: Change the then-current image-only invalidation form without adding vector resolution; close session-cache and retained-output borrows while preserving application ownership and accepted-operation publication, then close the user-authorized release-specific semantic and graph owners without changing later resolver/cache policy.
Verification Profile: `BEHAVIOR_CHANGE`
Change: Introduce generic target/all resource release plus a narrow identity-aware retained-output callback; migrate every producer, session/surface consumer, runtime delivery path, test double, and stable proof registration; contain notification failures after removal; retire image-named seams and private-shape mirrors; and align release-specific source owners, diagrams, the existing graph edge, and generated projections.
...
| `resource-seam-retirement` | Image-named declarations, implementations, exact method inventories, adapter shape mirrors, and stale app-key proof credit exist. | All producers and consumers migrate to generic release behavior. | No legacy declaration, consumer, compatibility alias, copied replacement inventory, or false proof registration remains. | Stable dependency-boundary and external app-key behavior proof remains; absence is one-time source evidence, not a permanent scanner. |
...
| `preparation-context-closure` | A real direction-sensitive raster-free vector and post-settlement context release have no preparation evidence. | Two preparations start under different effective text directions and a supplied context changes or unmounts during continuation. | Rendered output reflects invocation-time direction and no engine/completed-upstream path retains the supplied context after settlement. | Transient in-flight retention is allowed; locale/direction freshness remains separate caller evidence, and no production test-control seam is added. |
...
| `context-closure-evidence` | `preparation-context-closure` | `RUNTIME` | Real direction-sensitive raster-free vector under two effective text directions plus post-settlement context retaining-path observation. | No preparation output exists and caller cache identity cannot prove invocation-time direction or non-retention. | Direction-sensitive rendered output reflects invocation-time direction despite context change/unmount, and no completed owner retains the supplied context. | Direction-sensitive output and identity retaining paths are admissible; cache-key difference, locale freshness, one direction, source capture shape, or GC timing is rejected. | `ADD` | Preparation-owner context-sensitive raster fixture and VM retention artifact family | `context-sensitive-preparation-admission` |
...
| `application-no-dispose-evidence` | `application-assets-not-disposed` | `TEST` | Existing app-owned resource asset lifecycle coverage extended with prepared Picture identity observation. | Image non-disposal is covered, but vector wrapper/Picture engine non-disposal is absent. | Eviction/invalidation/replacement/reset/drop/dispose remove references without disposing either application asset family. | Native disposal observation is admissible; cache removal alone or wrapper boolean state is rejected. | `EXTEND_COVERAGE` | `test/resources/fixtures/app_owned_resource_asset_not_disposed_fixture.dart` and vector lifecycle owner family | `application-asset-ownership-admission` |
...
Impact: `ADD`
Failure family: preparation can use post-call/default direction or retain the supplied BuildContext after settlement
Failure mode or stable invariant: direction-sensitive output reflects invocation-time direction and completed owners retain no supplied context
Verification owner: public vector preparation context suite
Current verification gap: no direction-sensitive vector output or post-settlement retaining-path proof exists
Failing witness: the current public surface cannot prepare or render direction-sensitive vector text
Durable and refactor-stable value: direction output and settlement ownership survive preparation-adapter refactors
...
Artifact target: `test/resources/fixtures/app_owned_resource_asset_not_disposed_fixture.dart` and vector lifecycle owner family
