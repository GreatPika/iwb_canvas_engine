...
| `new-session-facts` | New-origin public identity or seed facts are incorrect or mistaken for committed-target authority | D-007 | Unit 10 | `new-session-facts-evidence` |
| `destination-query-work` | Pure destination lookup introduces or displaces full-owner work | R-015; Boundaries / Work Budget And Cost Displacement | Unit 8 | `destination-work-evidence` |
| `insertion-preparation-work` | Shared insertion preparation repeats projection or displaces work into delivery/cleanup | D-009, R-015; edit_kernel.md sparse preparation | Unit 9 | `insertion-work-evidence` |
...
| `destination-work-closure` | Named and omitted destinations over committed facts or an edit-local structure | Resolve a destination and perform the existing insertion | Pure lookup performs no document projection or full layer/element traversal; existing sparse/materialized insertion work remains within its current owner phase | No new order materialization during a committed read; existing edit-local order construction and necessary mutation/finalization work are neither repeated nor moved into lookup or cleanup |
...
| `insertion-work-closure` | Existing stroke/line routes over the established large-document work fixture | Prepare, accept or discard through the shared insertion handoff | Existing sparse preparation, exact-entry projection, one consume/install and cleanup work bounds are preserved | No CanvasDocument materialization, repeated candidate/entry projection or normalization, callback-multiplied work, or scene work moved into discard/cleanup; preserve existing owner-authorized finalization phases |
...
| `new-session-public-facts` | Existing text and a new seed with a nonzero revision; successive released new sessions | Admit through the public port, read the session, and confirm a valid creation or externally occupy its ID | Origin distinguishes existing/new; a new session reports the seed revision and generation zero; each admission has its own issued requestId and the creation request carries that same ID | Seed revision/generation are readable non-Store facts, not existing-target authorization; external occupation still causes retained stale with no existing-text update; observe actual sessions and requests, not constructed DTOs |
...
| `creation-evidence` | `atomic-text-creation` | `TEST` | Runtime prepared insertion through `test/runtime/fixtures/text_editing_port_fixture.dart` and stock new-origin overlay consumer in `test/surface/fixtures/text_editing_overlay_fixture.dart` | No transient text insertion or creation-request variant exists | One exact prepared creation entry/layer metadata is visible before document insertion; full final value matches accepted draft and geometry | Include unchanged nonempty seed, untrimmed nonempty input, prospective/existing layers, refusal and discard; change retained-layer contents after admission for omitted append and a numeric index that would normalize differently; derive expected placement from ordinary addElement over that preparation-time state, not the new helper; final text alone is insufficient | False positive: Request has fictional before value, creates a layer before acceptance, or freezes normalized index at admission; kill signal: Request/full installed placement disagrees with the preparation-time addElement oracle, resolver-time document or one-operation counts | `EXTEND_COVERAGE` | `test/runtime/fixtures/text_editing_port_fixture.dart` and `test/surface/fixtures/text_editing_overlay_fixture.dart` | `creation-evidence-admission` |
...
| `new-session-facts-evidence` | `new-session-public-facts` | `TEST` | Existing runtime text port fixture test/runtime/fixtures/text_editing_port_fixture.dart with bounded inspection of new/existing guard dispatch | New origin and seed guard facts are not available through the current port | Actual existing/new sessions distinguish origin, nonzero seed revision and generation zero; successive admissions use distinct issued requestIds that match actual creation requests; occupied prospective ID rejects creation as stale | Exercise public admission/read/finish and real external occupation; compile-only getters, constant IDs, forged DTOs and invented existing-target facts are rejected | False positive: Every session reports existing origin, a constant requestId or Store-like seed generation while creation still works; kill signal: Public value/request correspondence differs or occupation authorizes an existing-text update; inspect that existing-target authorization never consumes seed revision/generation | `EXTEND_COVERAGE` | `test/runtime/fixtures/text_editing_port_fixture.dart` | `new-session-facts-admission` |
| `destination-work-evidence` | `destination-work-closure` | `TEST` | Existing edit structural fixture test/edit/fixtures/sparse_edit_session/sparse_edit_session_structure_fixture.dart and its existing structural/projection observers; bounded selector/caller source inspection | Existing helpers provide no separately observable pure destination lookup phase | Isolated committed lookup has zero projection and full-order/layer/element traversal; existing edit-local lookup/insertion preserves the established structural work observations | Keep explicit host reads outside observation; use existing owner work probes and inspect all fact producers so work cannot hide before the selector; preserve required existing order builds in their original mutation phase | False positive: Correct lookup first scans all layers or materializes a document; kill signal: Query-phase projection/traversal work or a forbidden producer/caller scan is observed | `EXTEND_COVERAGE` | `test/edit/fixtures/sparse_edit_session/sparse_edit_session_structure_fixture.dart` | `destination-work-admission` |
| `insertion-work-evidence` | `insertion-work-closure` | `TEST` | Existing draw_commit_delivery_fixture large-document route/work observations, including _verifyAcceptedRouteReads and _expectAcceptedDrawWorkComposition; bounded inspection of both preparation consumers and discard/cleanup | Not required: REFACTOR retains the existing sparse candidate, projection, layer work, ID and delivery phase oracles | Existing no-projection, sparse preparation/consume, exact-entry and delivery work observations remain unchanged; both draw routes share that measured preparation path and retain bounded refusal/discard cleanup | Run the existing draw owner before and after; inspect actual shared preparation and cleanup for unobserved scans or repeated work; final element equality, timing and helper names alone are insufficient | False positive: Exact requests are correct after repeated candidate projection or cleanup scanning; kill signal: Existing projection/candidate/layer/delivery work oracle differs or changed-owner inspection finds repeated/displaced whole-owner work | `NONE` | None | None |
...
Failure mode or stable invariant: One exact prepared creation entry/layer metadata is visible before document insertion; full final value matches accepted draft and geometry, with numeric index/append normalized against the retained destination at preparation
...
Current verification gap: No transient text insertion or creation-request variant exists; no creation case changes destination contents between admission and preparation
Failing witness: No transient text insertion or creation-request variant exists; admission-time normalization could retain a stale append/numeric index after unrelated content changes
Durable and refactor-stable value: Consumer-visible results, state and failure boundaries remain falsifiable after private decomposition; include unchanged nonempty seed, untrimmed nonempty input, prospective/existing layers, refusal and discard; change destination contents between admission and finish and compare exact request/final placement with ordinary addElement over that preparation-time state; final text alone is insufficient
Artifact target: `test/runtime/fixtures/text_editing_port_fixture.dart` and `test/surface/fixtures/text_editing_overlay_fixture.dart`
...
### `destination-work-admission`: Pure destination query work

Covers: `destination-work-closure`
Impact: `EXTEND_COVERAGE`
Failure family: pure destination resolution introduces or displaces whole-owner work
Failure mode or stable invariant: committed destination lookup performs no document projection or full-order/layer/element traversal; edit-local selection preserves existing structural phase ownership
Verification owner: Existing edit structural placement fixture and owner work observers
Current verification gap: Existing mutation work tests do not isolate the newly separated read-only decision
Failing witness: Current combined selection/installation has no independent query-phase work oracle
Durable and refactor-stable value: Existing work observers and bounded producer/caller inspection reject scans hidden before the pure helper without coupling to its name or representation
Artifact target: `test/edit/fixtures/sparse_edit_session/sparse_edit_session_structure_fixture.dart`

### `new-session-facts-admission`: Public new-session facts

Covers: `new-session-public-facts`
Impact: `EXTEND_COVERAGE`
Failure family: new-session public identity and seed facts disagree with the admitted origin or are used as committed-target authorization
Failure mode or stable invariant: actual new sessions expose new origin, seed revision, generation zero and their issued requestId; creation guards remain distinct from existing-target authorization
Verification owner: Existing runtime text-editing port fixture
Current verification gap: Current sessions have no new origin or non-Store seed-fact lifetime
Failing witness: A constant/wrong origin, revision, generation or requestId can pass document-silence and compile checks
Durable and refactor-stable value: Public values, successive admission identity, actual creation-request correspondence and occupied-ID stale behavior remain observable without private representation assertions
Artifact target: `test/runtime/fixtures/text_editing_port_fixture.dart`
