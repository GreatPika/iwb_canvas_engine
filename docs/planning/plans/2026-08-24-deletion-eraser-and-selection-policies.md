...
The maintained runtime exposes the accepted deletion policies and pre-commit resolver API so selection deletion and terminal erasing derive one final canonical removal set, offer that complete set to a client before mutation, install an accepted deletion through the existing Store, Selection, EditKernel, CommitApplier, InteractionEngine, RuntimeRoot, and DiagnosticsHub owners, preserve current behavior when the resolver is absent, and keep public compatibility, bounded work, failure containment, cleanup, actions, documentation, and architecture projections consistent with the accepted design.
...
| `public-deletion-surface` | Public declarations, exports, defaults, and owning documentation can disagree. | R-004, R-012, R-013; D-002; A-002 | Units 1, 2, 4; Matrix | `public-deletion-surface-evidence` |
...
| `direct-port-migration` | The direct port extension can leave an implementer, migration note, or release surface incompatible. | R-012; D-002; A-005 | Unit 1; Matrix | `direct-port-migration-evidence` |
...
| `callback-entry-correctness` | Callback entries can be stale, incomplete, duplicated, misordered, or carry post-removal positions. | R-008; D-004; A-010 | Unit 3; Matrix | `callback-entry-evidence` |
...
| `resolver-null-fast-path` | Resolver-null routes can allocate callback/deferred state or change legacy behavior. | R-009; D-005; A-018 | Unit 4; Matrix | `two-route-null-fast-path-evidence` |
...
| `compatibility-workaround-excluded` | A V2 port, capability cast, duplicate getter, or runtime-state mirror can evade the accepted direct migration. | R-012, R-014; D-002, D-007 | Compatibility; Out of Scope; Matrix | `direct-port-migration-evidence` |
...
- `lib/src/contracts/public/canvas_runtime.dart:22` / public configuration owner: the const runtime configuration currently owns creation-time policies and has no deletion fields -> add each accepted field with its complete set of real consumers, preserving const/default behavior.
...
Compatibility: Optional configuration defaults preserve current selection partial deletion, unrestricted erasing, and resolver-null one-phase behavior; existing delete/erase action types, payload shapes, IDs, timestamp rules, document/resource formats, schemas, persisted values, and non-deletion public APIs remain unchanged. `CanvasSelectionPort.deleteAvailability` is the accepted source-breaking direct interface extension: runtime and the direct external compile consumer migrate in the same unit, the public contract records the migration, the release gate covers the new exact surface, no old surface is retired, and no compatibility workaround coexists.
Order Constraints: Units 1 and 2 are independent. Unit 3 consumes Unit 1 canonical selection facts while migrating both current ordering routes. Unit 4 consumes Units 1, 2, and 3 and atomically lands the complete public resolver surface, both public deletion consumers, the private prepare/consume seam, diagnostics, graph, cleanup ordering, and all route-local and cross-route evidence. Because Unit 4 is intentionally larger than the other units, implementation must pass through reviewable checkpoints on one unmerged change: first the shared public/private contracts, configuration, prepared-install boundary, guard, and diagnostic plumbing with their focused evidence; then the selection route with its route-local evidence; then terminal eraser routing and cleanup with its route-local evidence; finally cross-route, public-surface, documentation, graph, and lifecycle closure. These checkpoints are implementation-control boundaries, not independently acceptable contract units: none may be merged, released, or marked complete before the entire Unit 4 surface and evidence close together. Within each unit and checkpoint, direct evidence and authoritative documentation land with behavior; source truth and guardrails precede dependent consumers; preparation precedes callback; callback decision precedes install; accepted Store install precedes Selection install; eraser cleanup precedes fallible state/action delivery; lifecycle closure follows implementation evidence.
Temporal Surface Closure: The resolver is called synchronously exactly once only for a final nonempty set after all expected failures have been exhausted. Reads and client-owned Undo work remain permitted in the callback, while public runtime mutation/edit/tool calls, disposal, and nested resolvers are rejected without effects. Accept consumes the prepared state in the same uninterrupted stack; cancel and callback exception discard it; resolver absence retains the current one-phase route without request or deferred-state construction. Terminal eraser cleanup covers empty, rejected, preparation-failed, accepted, cancelled, resolver-failed, and delivery-failed branches before fallible publication; selection deletion never changes independent eraser interaction state.
...
| `eraser-policy-before-budgets` | Mixed-kind candidates surround candidate and exact-check limits in preview and terminal reads. | Reads run under null, empty, and explicit allow-lists with resolver absent and configured. | Disallowed kinds appear in neither preview nor terminal removals and consume neither budget; allowed kinds retain existing exact-hit semantics. | Preview and terminal use one policy; callback presence does not affect admission; `CanvasElementKind` remains the sole kind truth. |
...
| Outcome key | Starting state | System action | Observable result | Required side conditions |
| --- | --- | --- | --- | --- |
| `canonical-deletion-entries` | Final removal IDs are interleaved across multiple layers and may arrive canonical or permuted. | Selection and terminal eraser routes request Store batch projection before mutation. | Every final element appears once with its exact immutable element reference, source layer, original in-layer index, and document-layer/in-layer order. | One committed snapshot supplies all facts; no post-removal positions, duplicate order authority, public projection, or persistent index exists. |
...
Owner: Public deletion contracts and registry, RuntimeConfig, RuntimeRoot selection and terminal eraser coordination plus resolver guard, EditKernel, CommitApplier, Store and Selection installers, existing InteractionEngine cleanup owner, runtime diagnostics adapter, DiagnosticsHub code/contract, action finalizer, architecture graph/generated views, public/edit/runtime/diagnostic and eraser lifecycle documentation/diagrams, and their nearest route-local and cross-route verification owners.
Boundary: Land the remaining resolver configuration and five public resolver/request/entry/decision/operation declarations simultaneously with both accepted public consumers; introduce only the deletion-specific private prepare/single-use consume seam; complete all expected failures before either callback; reuse the existing guard; contain callback errors through the admitted diagnostic route; guarantee terminal cleanup before fallible delivery; migrate graph, direct public consumers, and all route-local and cross-route evidence atomically; keep resolver-null selection and terminal eraser deletion on their current one-phase paths. Execute this atomic unit through the mandatory shared-spine, selection-route, eraser-route, and final-cross-route checkpoints defined by Order Constraints, reviewing focused behavior and evidence at each checkpoint while keeping the whole unit unmerged until final closure.
Verification Profile: `BEHAVIOR_CHANGE`
Change: Public selection deletion and terminal eraser each present their complete Store-projected final set to the configured resolver before mutation, safely accept or veto it as one unit through existing owners, preserve terminal cleanup ordering, and retain legacy resolver-null behavior.

Acceptance Outcomes:

| Outcome key | Starting state | System action | Observable result | Required side conditions |
| --- | --- | --- | --- | --- |
| `deletion-resolver-public-surface` | The selection policy surface exists but resolver declarations/configuration do not. | Runtime and external clients compile against the complete accepted deletion API. | `deletionCommitResolver`, operation, decision, resolver, request, entry, and exact fields/defaults/exports/docs are present with no extras. | Request and entry use identity equality; config remains const; source-breaking getter migration from Unit 1 remains closed; no old surface retires or parallel alternative appears. |
| `deletion-request-immutability` | A mutable iterable contains entries holding immutable public element references. | A public request is constructed, the input mutates, and exposed-list mutation is attempted. | Request order/content remain unchanged, the list rejects mutation, and each exact element identity is retained. | One O(k) defensive copy occurs only for configured-resolver nonempty deletion. |
| `selection-resolver-decision` | Selection policy produces a final nonempty Store-projected set. | Runtime invokes the configured resolver under the existing guard and receives accept, cancel, or an ordinary exception. | Accept installs the whole prepared set; cancel installs none; exception does not escape and installs none. | Exactly one callback occurs; no engine Undo/rollback/public token exists; selection delete leaves independent eraser interaction state unchanged. |
| `selection-preparation-before-callback` | Each admitted document, selection, stale, validation, normalization, request, revision, and action-input failure can be injected at its owner. | Selection deletion prepares a configured-resolver operation. | Every expected failure occurs before resolver entry, with zero callbacks and unchanged committed effects. | No normal failure remains between accept and first owner installation; VM-fatal conditions are excluded. |
...
| `selection-resolver-null-fast-path` | Selection policy produces empty or nonempty sets while resolver is absent. | Public selection deletion runs. | Current one-phase effects/actions remain, policy still applies, and neither callback DTO nor deferred state is constructed. | Empty and rejected sets remain no-ops; current timestamp behavior is preserved. |
| `selection-action-compatibility` | Selection deletion is accepted with resolver absent or configured. | Runtime finalizes the committed action. | Existing `deleteElements` type and payload contain the installed IDs in accepted order with unchanged timestamp semantics. | No new deletion action or payload field is introduced; final state and action IDs agree. |
...
| `two-route-deletion-outcome` | Public selection and terminal eraser operations produce accepted, cancelled, empty, and policy-rejected sets. | Both routes run through a real runtime with resolver absent and configured. | Each nonempty configured route exposes the complete final set before mutation and accepts or vetoes it as one unit; empty/rejected routes do not call; existing owners install accepted work. | No engine Undo, rollback, public token, new coordinator, or route bypass exists. |
...
| `two-route-preparation-before-callback` | Every admitted preparation failure is injectable for both deletion routes. | Each route prepares a configured-resolver deletion. | Failure yields zero resolver calls and no committed effects; terminal eraser also completes cleanup. | No expected failure or public observation remains between accept and first install. |
...
| `two-route-resolver-null-fast-path` | Resolver is absent while selection and eraser policies vary. | Both public deletion routes run. | Current one-phase effects and legacy actions remain while no deletion request or deferred state is created. | Policies still apply; resolver-specific allocation is observed only at the new stable construction seam. |
| `two-route-action-compatibility` | Both deletion routes accept with resolver absent and configured. | Existing action finalization and public streams run. | Delete/erase action types, payload shapes, final IDs, and timestamp behavior match current contracts. | State precedes action; no new action type or callback-derived payload is exposed. |
| `two-route-resolver-cardinality` | Empty, invalid, rejected, accept, cancel, and throwing operations exist for both routes. | Each public operation runs one at a time. | Resolver count is zero for empty/invalid/rejected and exactly one for every nonempty accept/cancel/error operation. | Count observes the configured public resolver boundary, not a private helper; no retry occurs. |
...
| `direct-port-migration-evidence` | `selection-policy-public-surface` | `BUILD_OR_COMPILE` | Runtime implementation, direct external `CanvasSelectionPort` implementation, public contract, and release gate | The accepted direct getter extension is absent. | Both implementations compile against one extended interface, migration note/defaults are present, and no parallel port/getter/state mirror exists. | Direct compile plus authoritative docs is admissible; source absence or prose alone is rejected. | Runtime compiles via a capability cast while external implementation remains old; direct interface compile and forbidden-surface inspection kill it. | `EXTEND_COVERAGE` | `test/api_contract/public_api_v1_compiles_as_written_test.dart` and `docs/verification/release_gates.md` | `direct-port-migration-admission` |
...
| `eraser-prebudget-evidence` | `eraser-policy-before-budgets` | `TEST` | Existing preview/terminal eraser read boundary with stable candidate/exact counters | Every deletable kind currently consumes budgets and can be erased. | Null/empty/allow-list behavior matches in preview/terminal, disallowed candidates consume zero candidate/exact work, and resolver presence changes nothing. | Stable budget counters and public effects are admissible; removed-ID-only or wall-clock proof is rejected. | Filtering occurs after candidate counting but final IDs look correct; near-limit counters fail and kill it. | `EXTEND_COVERAGE` | `test/geometry/eraser_exact_budget_inputs_test.dart` and terminal eraser owner suite | `eraser-prebudget-admission` |
| `callback-entry-evidence` | `canonical-deletion-entries` | `TEST` | Store batch projection plus both real current route consumers | Current routes expose only ordered IDs and use full-handle ordering. | Interleaved multi-layer inputs yield exact element identities, layers, original indices, and order for both routes before mutation. | Direct Store facts on the same snapshot are admissible; action order or copied expected construction is rejected. | Entries are sorted but indices are recomputed after earlier removal; pre-mutation Store index comparison kills it. | `EXTEND_COVERAGE` | Store projection and runtime deletion-route owner-scoped fixture families; placement implementation-owned | `callback-entry-admission` |
...
| `public-deletion-surface-evidence` | `selection-policy-public-surface`, `deletion-resolver-public-surface` | `BUILD_OR_COMPILE` | Complete public compile/export/signature/default/docs surface after Unit 4 | All accepted deletion declarations are currently absent. | Every exact field, enum member, constructor/default, equality class, barrel export, registry entry, direct implementer, semantic signature, and migration note agrees with R-004/R-012/R-013. | Analyzer-resolved compile/export plus owner docs is admissible; any single inventory, wording token, or declaration-presence proxy is rejected. | All names export but a constructor default or request field differs; exact consumer construction and signature checks kill it. | `EXTEND_COVERAGE` | Existing public API compile/signature/export/equality owner suites | `public-deletion-surface-admission` |
...
| `selection-null-fast-path-evidence` | `selection-resolver-null-fast-path` | `TEST` | Public selection route plus narrow request/deferred construction counter | Current direct path exists but has no new seam to bypass. | Legacy effects/actions remain and construction counts are zero while policy applies. | Narrow stable construction seam and public effects are admissible; heap profiling/private names are rejected. | Shared helper always allocates request then skips callback; construction counter kills it. | `EXTEND_COVERAGE` | Selection deletion resolver owner-scoped runtime fixture family; placement implementation-owned | `selection-null-fast-path-admission` |
| `selection-action-evidence` | `selection-action-compatibility` | `TEST` | Real action finalizer and public action stream | Current selection delete emits existing action only on direct commit. | Resolver absent/accepted cases emit the same type, payload IDs, and timestamp behavior as installed state. | Real finalizer/stream is admissible; final document or declaration inspection is rejected. | Installed state is correct but callback entry IDs leak into a changed action order; stream comparison kills it. | `EXTEND_COVERAGE` | `test/interaction/commands_emit_user_actions_test.dart` and typed action owner | `selection-action-admission` |
...
| `two-route-null-fast-path-evidence` | `two-route-resolver-null-fast-path` | `TEST` | Both public routes plus request/deferred construction seam and public effects | Neither route currently has resolver-specific construction to bypass. | Both routes construct zero resolver-specific state and preserve legacy policy/action/effect behavior. | Narrow stable construction observation is admissible; final state alone or broad heap profiling is rejected. | Eraser eagerly builds entries/request then direct-installs; construction count kills it. | `EXTEND_COVERAGE` | Cross-route deletion lifecycle owner-scoped fixture family; placement implementation-owned | `two-route-null-fast-path-admission` |
| `two-route-action-evidence` | `two-route-action-compatibility` | `TEST` | Real action finalizer/public stream for both routes with resolver absent/accepted | Neither current route has resolver-aware action compatibility coverage. | Existing delete/erase types, payloads, final IDs, timestamp, and state-before-action order remain exact. | Real stream/finalizer is admissible; final state or declaration inspection is rejected. | Eraser emits correct final state but a new action type; stream type assertion kills it. | `EXTEND_COVERAGE` | `test/interaction/commands_emit_user_actions_test.dart`, typed action, and runtime delivery owners | `two-route-action-admission` |
| `resolver-diagnostic-evidence` | `selection-resolver-diagnostic`, `two-route-error-no-effect` | `TEST` | Both real resolver routes, diagnostics adapter/hub, control branches, disabled mode, and public non-export | No deletion-resolver diagnostic route currently exists. | Exactly one bounded record appears per thrown callback on both routes, controls remain silent, disabled mode allocates/retains none, and no public surface appears. | Real routes/hub/public barrel are admissible; enum presence or copied expected records are rejected. | Selection diagnostic is correct but eraser records twice or leaks entries; eraser cardinality/redaction kills it. | `EXTEND_COVERAGE` | `test/diagnostics/interaction_diagnostics_test.dart` plus cross-route deletion coverage | `resolver-diagnostic-admission` |
...
Failure mode or stable invariant: one exact exported selection policy and derived availability surface compiles for runtime and an external port implementer with legacy-preserving defaults
...
Artifact target: `test/geometry/eraser_exact_budget_inputs_test.dart` and terminal eraser owner suite

### `callback-entry-admission`: Canonical complete deletion entries

Covers: `canonical-deletion-entries`
Impact: `EXTEND_COVERAGE`
Failure family: Store projection can produce wrong element, layer, index, order, or route parity
Failure mode or stable invariant: both routes receive complete pre-mutation facts from one committed snapshot
Verification owner: Store projection and runtime deletion routes
Current verification gap: only ordered removal IDs exist
Failing witness: current public surface cannot construct a deletion entry
Durable and refactor-stable value: owner facts and public callback semantics survive sorting/helper refactors
Artifact target: Store projection and runtime deletion-route owner-scoped fixture families; placement implementation-owned
...
Impact: `EXTEND_COVERAGE`
Failure family: the exact accepted deletion declarations, fields, defaults, exports, and docs can diverge
Failure mode or stable invariant: one compiled/exported documented API matches R-004, R-012, and R-013 exactly
Verification owner: public API compile, signature, export, equality, and migration suites
Current verification gap: all deletion-specific declarations are absent
Failing witness: current consumer cannot name the accepted deletion resolver types
Durable and refactor-stable value: public compatibility remains protected across internal reorganization
Artifact target: Existing public API compile/signature/export/equality owner suites
...
### `selection-null-fast-path-admission`: Resolver-null selection fast path

Covers: `selection-resolver-null-fast-path`
Impact: `EXTEND_COVERAGE`
Failure family: resolver-null selection deletion can allocate deferred/request state or change legacy behavior
Failure mode or stable invariant: zero resolver-specific construction with existing one-phase effects and policies
Verification owner: selection deletion runtime route suite
Current verification gap: new construction seam does not exist to prove it remains unused
Failing witness: current code has no resolver-dependent branch
Durable and refactor-stable value: stable construction/effect boundary survives helper refactors
Artifact target: Selection deletion resolver owner-scoped runtime fixture family; placement implementation-owned
...
### `two-route-null-fast-path-admission`: Both resolver-null routes stay one-phase

Covers: `two-route-resolver-null-fast-path`
Impact: `EXTEND_COVERAGE`
Failure family: one route can allocate resolver state despite null configuration
Failure mode or stable invariant: both policies apply with zero request/deferred construction and legacy effects
Verification owner: cross-route deletion lifecycle suite
Current verification gap: terminal resolver-dependent construction seam does not exist
Failing witness: direct current behavior cannot detect future eager construction
Durable and refactor-stable value: stable construction counters survive private DTO/helper changes
Artifact target: Cross-route deletion lifecycle owner-scoped fixture family; placement implementation-owned
...
Covers: `selection-resolver-diagnostic`, `two-route-error-no-effect`
Impact: `EXTEND_COVERAGE`
...
| Canonical public route | Root barrel, public declarations, registry, semantic signatures, runtime adapter, and direct external implementer | Existing public API compile/export/signature/default/equality and direct-consumer checks selected by `docs/verification/release_gates.md` | All exact new names and fields are reachable once, both implementers compile, defaults/equality match, and no V2/capability/duplicate/state mirror appears. |
| Cross-unit deletion closure | Units 1-4 on one resulting repository state | Matrix evidence for `two-route-deletion-outcome`, `two-route-resolver-guard`, `two-route-preparation-before-callback`, `two-route-cancel-no-effect`, `two-route-error-no-effect`, `two-route-resolver-null-fast-path`, `two-route-action-compatibility`, `two-route-resolver-cardinality`, `two-route-layer-retention`, `two-route-resource-retention`, and `excluded-route-containment` | Every cross-route outcome passes through real public owners, all stop conditions remain false, and no unit-local proof is extrapolated to the other route. |
| Residual work-budget closure | Complete selection/eraser construction, read, projection, mutation/replay, install/publication, and cleanup/rollback routes | Direct Matrix counters for eraser admission, Store and end-to-end ordering, resolver-null construction, sparse mutation/replay, prepared install/publication, and discard/cleanup on the resulting repository state | Canonical k is linear, arbitrary k stays k-log-k, fixed-k is N-independent in every applicable phase, filtered eraser candidates spend no budget, resolver-null creates no resolver state, accepted install/publication has no duplicate pass, and cleanup performs no rollback or N-dependent work; DCM remains a separate maintainability signal and is not acceptance evidence. |
