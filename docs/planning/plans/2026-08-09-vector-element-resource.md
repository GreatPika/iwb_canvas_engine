...
- `git diff 5816c2f8b73629c0291c1f3471a77ee4b282ff86..HEAD` / baseline check: only the active design and active contract differ from the inspected repository baseline -> the design's exhaustive owner and mirror set has no implementation-owner drift.
...
Order Constraints: Unit 1 installs generic non-fallible release, migrates producers and consumers, clears session/output borrows before notification, and removes image-named seams only after all current paths adopt the replacement. Unit 2 first records Design D16 identity equality for lifecycle-owning `CanvasPreparedVector` in the current public API owner, then atomically adds `CanvasDataErrorCode.invalidVectorData`, implements the type and exact `prepareVector(ByteData, {BuildContext? context})` helper, and lands their public export-registry entries; preparation performs `vector.bytes` size admission, exact-range copy, invocation-time locale/direction capture, and `vg.loadPicture` invocation before first await, then selected-Future mapping at `vector.bytes`, intrinsic admission at `vector.intrinsicSize`, and wrapper publication. Unit 3 makes the current descriptor, final-candidate relationship, and immutable frame facts explicitly kind-aware while preserving image-only public and wire behavior. Unit 4 consumes Units 1 and 3 to generalize the current image-shaped request/result/cache/binding/output path into one family-neutral resource pipeline while preserving all image outcomes and current numeric policy. Unit 5 then adds the public sealed vector family plus `resourceKindMismatch` atomically across every production exhaustive branch, resolver implementation, binding, public registry entry, external fixture, and test double so the repository compiles; temporary interaction/render mirrors receive complete vector branches at this step. Its frame order is capture, record completion, resource pass, cache/liveness admission, synchronous resolution, immutable binding, clip/translate/scale, zero-or-one layer, `drawPicture`, restore. Relationship validation precedes codec DTO return, exported JSON return, and store install. Caller order is freshness check, immediate stale disposal without publication, or current publication before attach; replacement is prepare new, publish new, release every old alias/session/output, then dispose old. Only after direct seven-family interaction and selection outcomes pass does Unit 6 retire the interaction mirror and Unit 7 retire the frame mirror. Unit 8 updates remaining current semantic owners, graph, registries, ADR, proof inventory, and generated projections after implementation outcomes exist; lifecycle archival follows all evidence and finding closure.
