...
| S-042 | repository | `lib/src/contracts/public/canvas_contract_limits.dart` | Existing machine-consumable validation-limit owner used directly by interaction and other production boundaries |
...
| plan | none |
| user | S-003 |
| repository | S-004, S-005, S-006, S-007, S-008, S-009, S-010, S-011, S-012, S-013, S-014, S-015, S-016, S-017, S-018, S-019, S-020, S-021, S-022, S-023, S-024, S-025, S-026, S-027, S-028, S-029, S-030, S-031, S-032, S-033, S-034, S-035, S-036, S-037, S-038, S-039, S-040, S-041, S-042 |
| other | none |
...
| E-054 | S-042 | `lines 1-31` | The package centralizes machine-consumable validation constants in one declaration-only owner, and interaction already imports that owner for stroke capture limits. |
...
| R-002 | user_decision | Interaction owns one internal mutable retained corridor that appends distinct points without copy-on-append and never exposes mutable storage across a port or public boundary. Machine-consumable eraser soft-limit and resample-target constants are added to the existing `canvas_contract_limits.dart` owner with values 8000 and 4000, and `docs/contracts/validation_limits.md` documents those executable values rather than owning a second policy. Whenever an input corridor of length `n` exceeds the soft limit, its target output point at zero-based position `i` is the input point at index `(i * (n - 1)) ~/ (target - 1)` for every `i` from zero through `target - 1`; this floor-index mapping is the endpoint-preserving uniform resample and runs before further capture. Terminal exact hit and `corridorPointCount` operate on that retained approximation. The accepted cost is that discarded detours can miss narrow intersections and shortcut chords can create false positives. | S-003, E-015, E-028, E-040, E-043, E-045, E-046, E-054 | Concrete private type, identifiers, storage layout, allocation strategy, and loop structure remain open while preserving the exact index mapping; the 0.75-unit predecessor decimation is not included. |
...
| P-002 | Weak panels and arbitrarily long gestures require a stable upper bound without reviving private benchmark infrastructure. | S-003, E-015, E-023 | absorbed | D-001, A-001, A-012 | The design accepts approximation after overflow and uses deterministic owner-level counters to prove the work bound; it makes no frame-time, device-performance, or Flutter-route completion claim. |

## Decision Register

### D-001 — Bounded retained-corridor ownership and policy
- Concerns: `form`, `owner`, `source_of_truth`, `policy`, `dependency`, `state_data`
- Lock: `lib/src/contracts/public/canvas_contract_limits.dart` is the sole machine-consumable owner of the eraser soft limit 8000 and resample target 4000; `docs/contracts/validation_limits.md` documents those executable values and direct verification compares the two owners without introducing another policy registry. Interaction owns one mutable active eraser corridor from admitted down until centralized cleanup. Each distinct move or terminal sample is appended exactly once without copy-on-append; equal adjacent points are ignored. If an append produces an input corridor of length `n` above the executable soft limit, the same owner immediately replaces it with exactly `target` points where output position `i` selects input index `(i * (n - 1)) ~/ (target - 1)` for every zero-based `i`; no different rounding or tie rule is permitted. This mapping preserves the first point and newest endpoint before another sample can be admitted. No minimum-distance decimation is added. The retained corridor is the single source for immutable public preview snapshots, the immutable terminal request, terminal exact geometry, and public erase `corridorPointCount`; no raw-sample count or second exact trajectory is retained. `PointerSession` remains the unchanged passive internal carrier of the capture reference: it neither owns mutation nor snapshots/copies corridor points, and no production owner retains an obsolete session shell across synchronous engine mutation. Mutable storage never crosses the public API or InteractionReadPort. Existing dependency direction remains interaction capture to immutable read port, runtime adapter, geometry/spatial, and current runtime/edit delivery owners.
- Open: Concrete private capture type, field names, storage layout, allocation strategy, arithmetic implementation of the fixed index mapping, and focused helper placement remain open.
...
- Rationale: The boundary keeps the change performance-first and directly falsifiable through owner counters while making the selected accuracy loss visible instead of hiding an O(P) trajectory, relying on non-threshold profile completion, or inventing a new incremental semantics.

## Impact Register

### I-001 — Interaction eraser capture and route
- Action: update
- Surface: `lib/src/contracts/public/canvas_contract_limits.dart`; `lib/src/interaction/eraser_machine.dart`; `lib/src/interaction/interaction_engine.dart`
- Required by: D-001, D-002
- Resulting authority: D-001, D-002
- Contract requirement: Add the 8000 soft limit and 4000 resample target to the existing machine-consumable validation-limit owner, make interaction consume them directly, keep the validation document semantically aligned, and replace copy-on-append unbounded capture with the interaction-owned mutable retained corridor, exact duplicate/terminal admission, immediate endpoint-preserving resampling, immutable preview snapshots, no move-time read call, and one immutable bounded terminal read while preserving the committed single-append baseline and centralized cleanup order. Keep `PointerSession` unchanged as a passive same-reference carrier; do not add a second snapshot or corridor copy there.
...
- Claim: Overflow resampling deterministically selects 4000 points using input index `(i * (n - 1)) ~/ 3999` at each output position `i`, preserves its first point and newest endpoint, omits minimum-distance decimation, and makes that retained approximation the exact geometry and `corridorPointCount` truth.
...
- Oracle: Feed deterministic over-limit corridors through the real capture and terminal route; compare every retained source index with the fixed floor-index formula, including non-even spacing cases, then compare endpoints, preview corridor, terminal geometry input, `EraserCommitIntent`, `EraseActionIntent`, and public action point count and use separate narrow-detour and shortcut-chord witnesses to observe the documented possible miss and possible added hit after resampling.
...
### A-012 — Deterministic-counter performance acceptance
- Verifies: R-009
- Claim: Every performance assertion in this change is closed by deterministic counters at the real capture, move, terminal, and cleanup owners, including zero cleanup-phase read/envelope/query/exact or displaced corridor work, and no acceptance outcome depends on Flutter profile completion, generated summaries, elapsed time, a baseline, or a numeric threshold.
- Failure: A claimed bound is inferred from final output or timing instead of direct work events, a counter observes a proxy owner, or the future contract requires the official Flutter route or generated performance artifacts to accept the change.
- Oracle: Map each bounded-memory, copy-work, move-phase, terminal-phase, and cleanup-phase claim to its direct assertion-gated owner counter and inspect the future contract evidence set for absence of profile, timing, baseline, comparison-summary, and threshold dependencies.
- Proxy risk: Correct final geometry can hide repeated work, while successful profile completion can hide regressions and supplies no numeric performance verdict.
- Evidence constraints: Use deterministic retained counts and phase-specific work events at the real owners; do not substitute wall-clock measurements, generated profile artifacts, helper-call names, or restored benchmark infrastructure.
- Architecture seam: D-001
...
- Claim: Cancel, dispose, prepared load success, mode/tool change, active-session `interactive=false`, stale terminal, invalid terminal, empty/budget no-op, resolver cancel, resolver error, edit/preparation failure after an eraser intent, successful completion, and post-consume delivery failure all release active mutable eraser capture reachability through the existing centralized cleanup point appropriate to their phase.
- Failure: Any lifecycle, terminal, resolver, preparation, or delivery route leaves the active capture or obsolete session reachable, clears it after an externally observable delivery that requires prior cleanup, changes resolver-error or preparation-failure side semantics, or introduces a second cleanup owner.
- Oracle: Exercise every named lifecycle and failure exit through its real interaction, runtime, API, or surface owner and directly observe active-session/capture reachability plus cleanup ordering relative to load/install, resolver, preparation, diagnostic, and delivery events.
...
- Trigger: Direct A-001 work evidence plus a future product requirement or supported-environment observation shows that immutable preview snapshot/render work bounded at 8000 points still fails the required weak-panel experience and the required remedy changes public preview shape, publication cadence, or rendering ownership.
...
- ADR Impact: none
- Sources: S-001, S-002, S-003, S-004, S-005, S-006, S-007, S-008, S-009, S-010, S-011, S-012, S-013, S-014, S-015, S-016, S-017, S-018, S-019, S-020, S-021, S-022, S-023, S-024, S-025, S-026, S-027, S-028, S-029, S-030, S-031, S-032, S-033, S-034, S-035, S-036, S-037, S-038, S-039, S-040, S-041, S-042
