...
- `clear` and `clear(true)` remove ordinary-layer elements while preserving background/grid/background-element state; `clear(true)` retains image/vector descriptors still referenced by preserved background elements; both clear forms, resource operations, and later additions remain ordered barriers/actions;
...
Compatibility: preserve public signatures, accepted/no-op semantics, touched taxonomy, revisions, transient admissions, remove-unused results, final relationship semantics, exact error type/code/message/path/precedence, order behavior, promotion parity, serialization, projections, flat authoritative committed rows/orders, and immutable committed publication. The `clearContent` compatibility baseline is layer-only: clear removes every ordinary-layer element regardless of `isDeletable`, preserves background/grid/background-element values and order, and retains image/vector descriptors referenced by preserved background elements when unused resources are removed; accepted IDs, touched/revision facts, command facts/actions, and spatial state describe only actual removals. Private committed owners may add only the layer-location and split reference-count derived facts with the lifecycles and parity invariants locked here. The other authorized behavior correction is that failed draw/line preparation no longer consumes its speculative generated ID; successful generated-ID order and explicit generation behavior remain unchanged.

Order Constraints: the supplemental layer-only `clearContent` behavior contract lands after the completed authoritative-committed-facts contract and before performance Contracts 2-4, which treat that behavior as their compatibility baseline. Process journal entries strictly in order; interpret numeric indices sequentially with current clamping; record deferred validations and transient admissions in observable order; preserve layer-only clear as a barrier; preserve finalization gate order; perform relationship diagnostics in current selective/frame-order precedence; run the move resolver before ID/editor preparation; read the normalized draw/line cursor without reservation before prepare; advance it only through ordinary explicit reservation or accepted ledger admission; prepare selection before either accepted install branch; and preserve the changed-text active-session notification before common delivery.
...
| D20 | Adopt the explicit user-selected layer-only `clearContent` baseline before Contracts 2-4: clear ignores `isDeletable` for ordinary-layer removal, preserves background/grid/background elements, retains their referenced image/vector descriptors under unused-resource cleanup, and keeps actual-removal effects and spatial state exact; private decomposition remains open. | User decision recorded self-contained in Source Inputs; current clear paths remove background rows and all descriptors, while split image/vector ownership makes both retained-reference branches independently falsifiable. | Compatibility and order locks; clear parity, work-budget, resource-kind, effects, spatial, and handoff oracles. |
...
| Layer-only clear remains exact through the performance migration. | A backing removes background state, `isDeletable` blocks ordinary-layer removal, image/vector cleanup diverges, repeated selective descriptor removal multiplies resource-table work, or spatial delivery drops a preserved background element. | Cross-path clear traces compare final state, actual result/effect/action IDs, background/grid/order, image and vector retention, direct spatial queries, and owner counters proving bounded descriptor-table opening/copying/freezing for many-resource cleanup. | A vector-only fixture, final document equality, wall-clock timing, or touched flags alone can miss the image branch, quadratic map copying, stale facts/actions, or an empty spatial index. | Exercise materialized, sparse, promoted, direct-store, command, and runtime delivery owners; use deterministic semantic owner counters rather than helper names or timing; retain the existing genuinely-empty spatial reset case. |
...
- Update `docs/contracts/operation_matrix.md` to record the authorized failed draw/line ID-reuse correction and the layer-only clear baseline while preserving all other route cleanup, revision, repaint, action, request, and no-op effects.
- Update the `clearContent` behavior wording in `docs/contracts/public_api_v1.md` for layer-only removal, preserved background state, retained referenced descriptors, and actual-removal results/actions; preserve all other public behavior. Preserve `docs/contracts/codec_boundary.md` and `docs/contracts/validation_limits.md` behavior. Amend wording only if needed to clarify unchanged sequencing; no API shape, schema, diagnostic, or limit change is authorized.
...
Required contract partition: exactly four performance Change Contracts: (1) authoritative committed facts; (2) store transaction candidate; (3) indexed edit lifecycle and atomic install closure; and (4) runtime route and temporal delivery closure. The supplemental layer-only `clearContent` behavior contract lands after completed Contract 1 and before Contracts 2-4; it does not count as or absorb one of these four performance contracts.
