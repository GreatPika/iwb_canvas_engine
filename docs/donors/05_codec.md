...
| `lib/src/model/scene_builder_decode_scene_metadata.dart` | schema gate, camera/background/grid/palette validation sequence | `adapt` | old schema shape | P3 |
...
| `lib/src/model/scene_builder_decode_node_common.dart` | common id/revision/transform/flag/opacity/hitPadding decode | `adapt` | old common-field behavior may not be schema v1 behavior | P3 |
| `lib/src/model/scene_builder_decode_image.dart`, `*_path.dart`, `*_text.dart`, `*_stroke.dart`, `*_line.dart`, `*_rect.dart` | family decode validation and diagnostic paths | `adapt` | old JSON aliases are not preserved unless approved for schema v1 | P3 |
...
Old codec behavior to preserve as reference where it matches schema v1:
