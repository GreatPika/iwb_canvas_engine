...
|---|---|---|---|---|
| `lib/src/interactive/internal/scene_controller_scene_view_runtime.dart` | In the narrowed view/runtime graph: `in=0`, `out=3`; `SceneControllerSceneViewRenderState` has 25 methods and coupling 15 | The mixed render seam remains the primary hot spot in this family even though the boundary anchor itself stays upstream of the rest of the family graph | Runtime boundary plus split render-state family anchor | Primary |
| `lib/src/view/scene_view_runtime_host.dart` | In the narrowed view/runtime graph: `in=1`, `out=5`; host state coupling 17 | The host is graph-bounded but operationally central | Runtime host | Secondary |
| `lib/src/view/scene_view_render_surface.dart` | In the narrowed view/runtime graph: `in=1`, `out=5`; render-surface state coupling 15 | The surface is a bounded main-scene consumer, not the seam owner | Main-scene render surface | Secondary |
| `lib/src/view/scene_view_interactive_overlay_painter.dart` | In the narrowed view/runtime graph: `in=1`, `out=1` | Bounded overlay consumer with one contract dependency on the mixed render-state seam | Overlay preview consumer | Keep local |
| `lib/src/render/scene_painter.dart` | In the narrowed view/runtime graph: `in=1`, `out=12`; imports 15 | Main-scene paint anchor under the render side | Main-scene painter anchor | Keep local |
| `lib/src/render/scene_painter_frame.dart` | In the narrowed view/runtime graph: `in=2`, `out=4`; `ScenePainterFrameOwner` coupling 16 | Hot render-side support owner, but still downstream of the main-scene read side | Frame-preparation support owner | Keep local |
...
| Overlay preview consumer | `lib/src/view/scene_view_interactive_overlay_painter.dart` | DCM shows a bounded overlay consumer. In the narrowed family graph it depends only on the render-state seam and does not become a second seam owner. |
