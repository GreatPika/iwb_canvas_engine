...
| `RuntimeRoot` through `RuntimeInteractionDiagnosticsAdapter` | An ordinary synchronous deletion resolver exception | `interaction` | `DiagnosticSeverity.warning` plus internal `InteractionDiagnosticCode.deletionResolverFailed` with no public API export | Exactly `operation` and bounded `errorKind` facts: `error`, `exception`, or `object`. No exception message, runtime type, element, document, or request data is admitted. | Records once after containment; disabled diagnostics skip record and detail work. Accept, cancel, and preparation failures do not use this writer. | implemented | Graph edge `runtime.deletion_resolver_failure.reports_to_diagnostics` and deletion-route diagnostics coverage. |
