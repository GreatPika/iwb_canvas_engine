...
| `RuntimeRoot` through `RuntimeInteractionDiagnosticsAdapter` | An ordinary synchronous deletion or pointer-Move resolver exception | `interaction` | `DiagnosticSeverity.warning` plus internal `InteractionDiagnosticCode.resolverCallbackFailed` with no public API export | Exactly `operation` and bounded `errorKind` facts: `error`, `exception`, or `object`. No exception message, runtime type, element, document, or request data is admitted. | Records once after containment; disabled diagnostics skip event and detail work. A diagnostics-recording fault is contained too. Accept, cancel, preparation failures, and already-recorded guard rejections do not use this writer. | implemented | Deletion-route and selected-Move callback diagnostics coverage. |
