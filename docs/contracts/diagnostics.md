...
| `RuntimeRoot` through `RuntimeInteractionDiagnosticsAdapter` | An ordinary synchronous unified commit resolver or lease callback exception | `interaction` | `DiagnosticSeverity.warning` plus internal `InteractionDiagnosticCode.resolverCallbackFailed` with no public API export | Exactly the bounded operation name (`draw`, `delete`, `erase`, `move`, `rotate`, `reflect`, `textEdit`, `commitLeaseCommitted`, or `commitLeaseAborted`) and `errorKind`: `error`, `exception`, or `object`. No exception message, runtime type, element, document, or request data is admitted. | Records once after containment; disabled diagnostics skip event and detail work. A diagnostics-recording fault is contained too. Accept, cancel, preparation failures, and already-recorded guard rejections do not use this writer. | implemented | Unified commit and lease callback diagnostics coverage. |
