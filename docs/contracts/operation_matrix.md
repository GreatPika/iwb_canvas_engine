...
| selected move commit | transforms | state.revisions.document, state.revisions.preview if active preview cleared; internal bounds, elementVisual, projection | touched update | evict | RuntimeRoot resolves configured move callbacks before preparation; after closed apply it performs publish-false main cleanup before common delivery | moveSelection after public state; `runtime_created_timestamps_monotonic` |
...
| eraser commit | removed elements plus selection-owner prune when erased ids intersect selection | state.revisions.document, state.revisions.selection if pruned, state.revisions.preview if active preview cleared; internal structural, bounds, elementVisual, projection | remove ids | evict | closed apply -> publish-false cleanup/effect merge -> common delivery, main + overlay | erase after public state if removed; `runtime_created_timestamps_monotonic` |
