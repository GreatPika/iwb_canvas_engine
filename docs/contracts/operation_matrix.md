...
| pencil/marker commit | add stroke | state.revisions.document, state.revisions.preview if active preview cleared; internal structural, bounds, elementVisual, projection | accepted Store-ledger admission reserves the RuntimeRoot-read candidate; failed/no-op preparation leaves it next | evict | main + overlay cleanup | drawPencil/drawMarker; `runtime_created_timestamps_monotonic` |
...
| line commit | add line | state.revisions.document, state.revisions.preview if active preview cleared; internal structural, bounds, elementVisual, projection | accepted Store-ledger admission reserves the RuntimeRoot-read candidate; failed/no-op preparation leaves it next | evict | main + overlay cleanup | drawLine; `runtime_created_timestamps_monotonic` |
