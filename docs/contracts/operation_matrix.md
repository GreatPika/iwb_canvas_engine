...
| command removeElement | registry/layer membership, plus selection-owner prune when removed id was selected | state.revisions.document, state.revisions.selection if pruned; internal structural, bounds, elementVisual, projection | remove id | evict | prepares the complete content or background Delete request before unified confirmation; cancel/error/pre-install failure leaves state unchanged, accepted install enters common delivery | deleteElements only after accepted state and lease commitment; `runtime_created_timestamps_monotonic` |
...
| selected move commit | retained participant transforms | state.revisions.document, state.revisions.preview if active preview cleared; internal bounds, elementVisual, projection | touched update | evict | Accepted participant touches, same-ID replacement, document replacement, or external selection outcome clear the session before public delivery; unrelated/no-op edits retain it. RuntimeRoot resolves one `CanvasMoveCommitRequest` before the shared Runtime-local transform/action preparation; after closed apply it performs publish-false main cleanup before common delivery | moveSelection after public state and lease commitment; `runtime_created_timestamps_monotonic` |
| rotate/flip selection | immutable current command participants, final pivoted world transform | state.revisions.document; internal bounds, elementVisual, projection | touched update | evict | RuntimeRoot resolves the qualified `CanvasRotateCommitRequest` or `CanvasReflectCommitRequest` before the same Runtime-local transform/action preparation; generic accept installs, cancel/error discard without commit | transformSelection after public state and lease commitment; `runtime_created_timestamps_monotonic` |
| deleteSelection | eligible selected elements/layers plus selection-owner prune after guarded unified confirmation | state.revisions.document, state.revisions.selection; internal structural, bounds, elementVisual, projection | remove canonical Store entry ids | evict | prepare full sparse/selection/revision/action state before `CanvasDeleteCommitRequest`; generic accept installs bound Store then owned Selection once, cancel/error discard without commit | deleteElements after accepted state and lease commitment; `runtime_created_timestamps_monotonic` |
...
| pencil/marker commit | add stroke | state.revisions.document, state.revisions.preview if active preview cleared; internal structural, bounds, elementVisual, projection | accepted Store-ledger admission reserves the RuntimeRoot-read candidate; failed/no-op preparation leaves it next | evict | qualified prepared `CanvasDrawCommitRequest` resolves before consume; closed apply -> publish-false cleanup/effect merge -> common delivery, main + overlay | drawPencil/drawMarker after public state and lease commitment; `runtime_created_timestamps_monotonic` |
...
| line commit | add line | state.revisions.document, state.revisions.preview if active preview cleared; internal structural, bounds, elementVisual, projection | accepted Store-ledger admission reserves the RuntimeRoot-read candidate; failed/no-op preparation leaves it next | evict | qualified prepared `CanvasDrawCommitRequest` resolves before consume; closed apply -> publish-false cleanup/effect merge -> common delivery, main + overlay | drawLine after public state and lease commitment; `runtime_created_timestamps_monotonic` |
...
| commitTextEdit policy empty deletion accepted | an active session captured with deleteElement and a trim-empty terminal draft selects the shared direct-removal preparation before draft equality; it resolves one `CanvasDeleteCommitRequest` containing the Store-projected original entry and exact placement, then consumes the closed package, removes the matching session, and enters common delivery | state.revisions.document and, only for a matching active session, state.revisions.interaction; internal structural, bounds, elementVisual, projection | remove original id; no empty-layer cleanup | evict | cancel, resolver failure, and pre-install failure retain original element/session draft; accepted delivery commits its lease before one deleteElements action and matching-session close notification | deleteElements; `runtime_created_timestamps_monotonic` |
| commitTextEdit changed accepted | text element content through EditKernel; keepElement empty input and every nonempty terminal preserve the untrimmed candidate, resolve `CanvasTextEditCommitRequest`, then after EditKernel closure RuntimeRoot consumes/removes the live request, silently clears only a matching active text session and its owned suppression/candidate state, and records an interaction revision before capture; it completes guarded common delivery, releases the guard, then notifies matching-session closure before true return | state.revisions.document and, only for a matching active session, state.revisions.interaction; internal bounds when layout bounds change, elementVisual, projection | touched update when text layout bounds change; none otherwise | evict | main common delivery, lease commitment after public state and before action/observer, then matching-session close notification; a listener may start another session without losing it | editText; `runtime_created_timestamps_monotonic` |
| CanvasTextEditingPort.startNew accepted | retains a text seed, selected destination and raw insertion index only in the runtime session; it installs no element, layer, selection or document change | state.revisions.interaction | none | no | stock overlay observes the new session without committed-text paint suppression | none |
| CanvasTextEditingPort.finishActive new nonempty accepted | validates the separate creation guard, prepares one insertion through the shared closed insertion handoff, resolves `CanvasTextCreateCommitRequest`, then consumes/removes the creation request and matching session before common delivery | state.revisions.document and interaction; internal structural, bounds, elementVisual, projection | accepted insertion only | evict | common delivery, lease and identity-safe close match changed text edit | createText; `runtime_created_timestamps_monotonic` |
| CanvasTextEditingPort.finishActive new trim-empty | consumes the creation request and closes the matching session before preparation or resolver | state.revisions.interaction | none | no | no action, lease, layer or document effect | none |
| CanvasTextEditingPort.finishActive cancel | closes the matching active draft, including a stale draft, without document preparation or guard validation | state.revisions.interaction when an active session closes | none | no | frame suppression ends and the active-session listener observes closure | none |
| CanvasTextEditingPort.finishActive rejected/noActiveSession | resolver refusal retains the active retryable draft; an absent session reports noActiveSession | none | none | no | no document, action, or session-close effect for noActiveSession | none |
...
- `commitTextEdit` validates `newText` before request consumption and before
  ...
  raw text.
- Changed text or B/I/U formatting commits may update the target text element
  transform to preserve the resolved horizontal text anchor and top edit edge
  when the complete draft's measured bounds change.
