...
| `core.no_retired_package_imports` | `parsed_ast_directive` | `negative_legacy_shape` | `strong legacy precedent` | Scan imports and package paths; add retired-shape checks when copied retired public/runtime concepts can reappear. |
...
| `core.no_retired_controller_shape_dependency` | `negative_legacy_shape` | `resolved_element_identity` | `derived from legacy pattern` | Reject the retired controller concept by public symbol, import, and resolved owner when needed. |
| `core.no_retired_node_patch_shape_dependency` | `negative_legacy_shape` | `resolved_element_identity` | `derived from legacy pattern` | Reject retired NodeSpec/NodePatch/PatchField shapes beyond text-only names. |
...
| `geometry.committed_handle_order` | `behavioral_seam_test` | `negative_legacy_shape` | `derived from legacy pattern` | Prove accepted current hit order and keep a retired-shape check for scene-order reintroduction. |
...
| `cache.keys_use_next_revisions_only` | `behavioral_seam_test` | `registry_parity`, `negative_legacy_shape` | `derived from legacy pattern` | Prove actual cache keys use next-owned revision facts, match cache policy rows, and avoid non-owned snapshot shapes. |
