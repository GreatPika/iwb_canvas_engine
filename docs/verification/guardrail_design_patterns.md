...
| `diagnostics.sanitized_public_projection` | `behavioral_seam_test` | `registry_parity`, `resolved_public_surface` | `derived from legacy pattern` | Prove details are bounded public data, select diagnostics-facing public declarations from `diagnostics_public_surface`, and traverse resolved public signatures so runtime objects cannot bypass the guard through non-prefix names. |
