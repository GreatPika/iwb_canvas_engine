...
| `release.benchmark_readiness` | GitHub release benchmark workflows are quarantined while device/manual references own performance comparison; public/runtime source cannot expose benchmark tooling, public integration names, benchmark route outside release policy, benchmark-only production hooks, workflow benchmark commands, or workflow approved-baseline writes |
