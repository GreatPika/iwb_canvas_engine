...
- `docs/contracts/operation_matrix.md` / `loadDocumentFromJson success`: load success touches prepared interaction cleanup outcome, and after successful install runtime consumes the already prepared outcome without a post-install interaction owner boundary -> effect and repaint aggregation must use the prepared outcome.
