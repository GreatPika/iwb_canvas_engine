...
design_question: "Run the architecture-design-workflow for docs/implementation/p5_edit_core.md and decide the architecture form for P5 edit core before a future Change Contract."
---
...
interaction tools, load-document staging, frame rendering, resources, spatial
indexes, or Flutter surface behavior in this design. Those future owners should
consume typed edit effects only after P5 defines the atomic commit boundary.
...
The public edit API shape already exists; P5 turns the existing unimplemented
edit surface into working runtime behavior without changing public signatures.
...
- `lib/src/api/canvas_runtime.dart:41` - `CanvasRuntime.edits` exists but is
...
- `lib/src/runtime/runtime_root.dart:36` - `RuntimeRoot` composes
  ...
  state notifier.
- `lib/src/runtime/runtime_root.dart:246` - selection-owned document mutation is
  currently rejected until later edit phases.
- `lib/src/runtime/runtime_root.dart:274` - `RuntimeRoot` publishes public state
...
- Form: add an edit-owned transaction subsystem under `lib/src/edit/**`, expose
  ...
  effects at the runtime boundary and returns/publishes typed post-install
  effects without importing concrete frame, spatial, resource, or Flutter
  owners.
...
| Frame, spatial, projection, and resource owners will need typed effects but must not become compile-time dependencies of `CommitCompiler`. | `docs/contracts/edit_kernel.md:147`; `docs/implementation/p5_edit_core.md:106` | `CommitCompiler` emits typed effect descriptions, and `CommitApplier`/runtime dispatch owns post-install delivery. | P5 must define effect DTOs carefully enough for tests while accepting that later owners may extend dispatch handling. |
...
atomically, then exactly one public `CanvasRuntimeState` is published when
public revisions change, and typed post-install effects are dispatched or made
observable through future owner boundaries.
...
| Boundary | pass | Entry boundary is `CanvasEditPort.edit`; exit boundaries are `CommitPlan`, `CommitApplier`, committed store/selection installs, typed post-install effects, and public `CanvasRuntimeState` publication: `docs/contracts/edit_kernel.md:65`, `docs/contracts/edit_kernel.md:76`, `docs/contracts/edit_kernel.md:77`, `docs/contracts/edit_kernel.md:82`, `docs/contracts/edit_kernel.md:208`. |
...
| State/data | pass | Committed document/revision/projection state stays in `DocumentStoreKernel`; selection state stays in `SelectionKernel`; public state stays in `RuntimeRoot`; draft/session/buffers/touched/effects are edit-owned transient state: `lib/src/store/document_store_kernel.dart:13`, `lib/src/selection/selection_kernel.dart:7`, `lib/src/runtime/runtime_root.dart:274`, `docs/architecture/01_runtime_ownership.md:62`. |
| Seam | pass | The new shared seam is `EditKernel` behind the existing `CanvasEditPort`; future commands and interaction become consumers, and direct mutation placeholders such as `rejectSelectionDocumentMutation` retire when their behavior routes through edit commit: `lib/src/api/canvas_runtime.dart:41`, `lib/src/runtime/runtime_root.dart:246`, `docs/contracts/public_api_v1.md:1474`. |
| Verification | pass | P5 has focused edit tests and guardrail ids for sync/non-nested/async/stale, rollback, operation matrix effects, exact touched invalidation, typed effects, low-level event silence, runtime publication, and selection owner separation: `docs/implementation/p5_edit_core.md:73`, `docs/verification/tests.md:263`, `docs/verification/tests.md:478`, `docs/verification/tests.md:508`. |
| Future pressure | pass | P6/P7/P10-P12 and command/event future pressures are absorbed by the transaction seam while deferring owner-specific behavior to later phases: `docs/contracts/operation_matrix.md:45`, `docs/contracts/public_api_v1.md:1769`, `docs/contracts/public_api_v1.md:1411`, `docs/implementation/p5_edit_core.md:112`. |
...
- Seam: `CanvasEditPort.edit -> EditKernel -> DraftDocument -> CommitCompiler
  ...
  -> RuntimeRoot public state/effect dispatch`.
...
- Exit boundaries: committed document install, selection effect install, one
  ...
  typed post-install effects for frame/spatial/resource/projection/repaint
  owners, and low-level edits with no user action events.
...
- Execution order constraints: reject disposed/nested sessions before draft
  ...
  selection effects atomically; publish state/effects/events only after install;
  close and stale the handle on success or rollback.
...
- Verification strategy: focused edit tests, field-update validation tests,
  ...
  invalidation except replacement, and public API/smoke tests using only the
  public barrel.
...
| Does it change public API consumer flow, payload shape, or compatibility behavior? | yes | sequence | The existing `CanvasRuntime.edits` flow changes from unimplemented to synchronous transactional behavior without changing signatures. |
...
C4Component
  ...
  Component(applier, "CommitApplier", "Runtime/Edit boundary", "Atomic install and effect dispatch")
  ...
  Rel(applier, runtime, "publishes one state snapshot when changed")
  Rel(applier, effects, "emits typed post-install effects")
```
...
flowchart TD
  ...
  Effects["Post-install typed effects\nprojection/spatial/resource/repaint/frame owners"]
...
sequenceDiagram
  ...
  Applier-->>Runtime: typed post-install effects
...
stateDiagram-v2
  ...
  Applying --> Closed: atomic install and post-install publication complete
...
- `docs/contracts/edit_kernel.md`, `docs/contracts/operation_matrix.md`,
  ...
  only if implementation finds a contradiction in the existing source of truth.
...
invalidation. Structural guardrails should ensure `CommitCompiler` does not
import concrete `FrameEngine` or other downstream owners and that generic global
invalidation appears only for document replacement.
...
- Evidence to cite:
  ...
  - current composition and placeholders:
    ...
    `lib/src/runtime/runtime_root.dart:274`.
