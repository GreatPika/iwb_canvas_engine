...
- `.research/2026-06-14-resource-image-cache-memory-accounting.md` - confirms current `ImageResolveCache` entry-only capacity, descriptor `byteLength` flow, app-owned image lifecycle, current tests, and absence of byte-capacity logic in `lib/src/resources/**`.
