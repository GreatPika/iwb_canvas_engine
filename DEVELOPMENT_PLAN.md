...
1) **Вынести общие правила проверки чисел/геометрии в отдельный внутренний модуль**  
...
2) **Сделать один строгий конструктор `Snapshot -> Scene` для store**  
...
3) **Переписать сериализацию на прямую работу из `Scene`**  
   `encodeScene(...)` должен строить JSON напрямую из `Scene`, без шага `Snapshot -> Scene`.
4) **Замкнуть все “входы сцены” на строгий конструктор**  
...
5) **Тесты на запрет “внутренне порченого” initial/replace**  
...
1) **Валидация на `txnNodeFromSpec` и `txnApplyNodePatch`**  
   ...
   * для `Patch`: проверять только присутствующие поля (`PatchField.value`), разумеется теми же правилами.
   ...
   Отдельно: `opacity` можно оставить «мягким» (нормализация уже встроена в узлы, и это явно задокументировано в модели узлов). 
2) **Закрыть прямой обход через writer-методы, принимающие `Transform2D`/`Offset`**  
...
3) **Тесты на отказ для невалидных Transform2D/Offset**  
   В существующих тестах уже проверяется отказ для камеры/сетки. Этот же стиль тестов должен быть расширен на трансформы/смещения. fileciteturn38file1L1-L1
