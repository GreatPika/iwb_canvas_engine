...
### [x] Этап 9. Убрать пересоздание множеств в горячих местах (обязательно)

**Цель:** минимизировать аллокации и исключить квадратичность на сериях операций.

**Действия:**

- [x] 9.1. В `TxnContext` сделать `workingSelection` мутабельным (`HashSet`), как вы уже планировали для `workingNodeIds`.

- [x] 9.2. В `SceneWriter` заменить:

* пересборку `_ctx.workingSelection = <NodeId>{ for (...) ... }`
* и `writeSelectionToggle` с созданием новых Set
  на обычные `.add/.remove`.

- [x] 9.3. Переписать `ChangeSet` так, чтобы `txnTrackAdded/Removed/Updated` мутировали внутренние `HashSet`, а не создавали новые множества.

- [x] **Критерий готовности:** массовые операции (1000 add/remove/toggle) не дают лавинообразного роста выделений памяти.
...
### [x] Этап 11. Ввести жёсткие пределы на “плохие” данные и худшие случаи (обязательно)

**Цель:** движок не должен зависать на одном плохом объекте/жесте.

**Действия:**

- [x] 11.1. Ограничение на stroke points:
...
- [x] 11.2. Ограничение на сложность `_hitTestPathStrokePrecise`:
...
- [x] 11.3. Ограничение на SpatialIndex.query по размеру запроса:
...
- [x] **Критерий готовности:** ни один одиночный объект/жест не может породить неограниченные двойные циклы по координатам/метрикам/ячейкам.

---

### [x] Этап 12. Закрыть “ручные заплатки” TextNode.size (обязательно для заявленной цели)
...
- [x] 12.1. Ввести единый механизм измерения текста (внутри движка):
...
- [x] 12.2. Сделать `TextNode.size` **производным полем**:
...
- [x] 12.3. Увязать это с `hitGeometryRevision/renderGeometryRevision`:

* изменение текста/стиля ⇒ меняются и hitBounds, и renderBounds (через ревизии).

- [x] **Критерий готовности:** поменяли стиль текста — попадание/выбор/отрисовка корректны без внешнего обновления размеров.
