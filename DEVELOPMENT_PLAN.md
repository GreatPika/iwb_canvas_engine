...
This file tracks prioritized follow-ups after P0 ("strict numeric validation + normalizeToLocalCenter preconditions").

### Important (P1 — after P0 or when jank appears)

- [ ] Cache text layout (`TextPainter`/layout) for `TextNode` to avoid recomputing text every frame.
- [ ] Cache draw path for `StrokeNode` to avoid rebuilding `Path` every frame for long or numerous strokes.
- [ ] Clarify and separate `selectionRect` updates vs selection/scene "revisions" so selection state is predictable for optimizations and maintenance.
