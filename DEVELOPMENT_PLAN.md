...
* [ ] В `lib/src/render/render_geometry_cache.dart` найти `_buildValidityKey` для `StrokeNodeSnapshot`.
* [ ] Убрать `strokeNode.points` из ключа валидности (оставить `pointsRevision`, толщину, transform и прочие скалярные поля).
* [ ] Проверить, что `pointsRevision` действительно увеличивается при изменении точек (по коду вставки/патча/коммита штриха).
* [ ] Добавить тест/проверку производительности/кэш-попаданий:

  * [ ] два последовательных `snapshot` без изменения конкретного stroke не должны приводить к пересчёту геометрии этого stroke.

---

### C. Жизненный цикл кэшей в `SceneViewV2`: geometry vs остальные

* [ ] В `lib/src/view/scene_view.dart` зафиксировать текущую схему:

  * [ ] кэши `static/text/stroke/pathMetrics` создаются/передаются и очищаются в `SceneViewV2State`.
  * [ ] `RenderGeometryCache` создаётся/живёт внутри `ScenePainterV2`.
* [ ] Принять решение по архитектуре владения (одно):

  * [ ] **Унифицировать**: передавать `geometryCache` в `ScenePainterV2` из `SceneViewV2State`.
...
* [ ] Добавить краткое описание правил владения ресурсами в руководство/комментарии (кто чистит, когда чистит, как избежать утечек).
...
* [ ] Для каждого изменения выше добавить/обновить инвариант/комментарий рядом с кодом:
  ...
  * [ ] “валидность stroke определяется `pointsRevision`, список точек в ключ не входит”
