...
- [ ] Сверить структуру папок пакета с deliverables (`lib/core`, `lib/render`, `lib/input`, `lib/serialization`).

## Этап 1 — Core модель и математика

- [ ] Описать базовую модель `Scene`, `Layer`, `Node` и типы узлов.
- [ ] Реализовать геометрию: AABB, поворот вокруг точки, отражение относительно оси.
- [ ] Реализовать пересчёт координат с учётом камеры.
- [ ] Реализовать hit‑test: прямоугольник, линия/штрих с допуском толщины.
- [ ] Добавить юнит‑тесты для математики и hit‑test.

## Этап 2 — JSON сериализация (v1)

- [ ] Схема JSON с `schemaVersion = 1`.
- [ ] Экспорт/импорт сцены и узлов (включая `ImageNode` и `TextNode`).
- [ ] Валидация входного JSON и понятные ошибки.
- [ ] Юнит‑тесты сериализации/десериализации.

## Этап 3 — Отрисовка

- [ ] Реализовать `ScenePainter`: фон, сетка, слои, узлы.
- [ ] Отрисовка узлов: image, text, stroke, line, rect.
- [ ] Отрисовка выделения и рамки выделения.
