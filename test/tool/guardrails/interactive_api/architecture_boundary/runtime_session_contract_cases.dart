...
''',
  ...
  test(
    'rejects interaction runtime that re-exposes move preview readback wrappers',
    () async {
      final sandbox = await createGuardrailsSandbox();
      try {
        writeMinimalControllerStore(sandbox);
...
extension SceneControllerInteractionRuntimeMutationApi
  ...
  Offset previewDeltaForNode(Object nodeId) => Offset.zero;

  SceneViewFramePreview captureFramePreview() => throw UnimplementedError();
...
''',
        ...
        expect(
          ...
          diagnostic(
            category: 'interactive API',
            detail:
                'SceneControllerInteractionRuntime must not re-expose move '
                'preview readback; composition wiring must use '
