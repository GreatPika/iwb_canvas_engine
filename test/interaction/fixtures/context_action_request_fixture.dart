...
void _expectContentRequest(CanvasContextActionRequested request) {
  expect(request.trigger, CanvasContextActionTrigger.doubleTap);
  expect(request.timestampMs, 7);
  expect(request.viewPosition, const Offset(20, 0));
  expect(request.worldPosition, const Offset(20, 0));
...
final class _RuntimeContextRequestScenario {
  _RuntimeContextRequestScenario({
    ...
    root = RuntimeRoot(
      initialDocument: _document(selectableContent: selectableContent),
      config: config,
...
CanvasDocument _document({required bool selectableContent}) {
