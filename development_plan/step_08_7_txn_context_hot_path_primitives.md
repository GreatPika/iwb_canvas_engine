...
[ ] Перевести cloned-layer bookkeeping на identity, которая не требует
    последующего index shift после вставки слоя.
[ ] Вынести locator shift после layer insert в narrow helper рядом с
    scene/locator primitives или переиспользовать уже существующий helper, если
    он покрывает нужную семантику.
[ ] Схлопнуть `_txnBaseNodeById(...)` до reuse locator-based lookup logic.
[ ] Разделить `txnResolveMutableNode(...)` на узкие стадии подготовки mutable
    container/slot и clone decision без изменения COW semantics.
[ ] Явно зафиксировать или переиспользовать invariant уникальности content
    layer ids там, где cloned-layer bookkeeping переводится на `LayerId`.
[ ] Повторно снять диагностические метрики по `TxnContext` watchpoint-owner-ам
    и убедиться, что сложность исчезла по ownership, а не просто сменила имя.

## Критерии приёмки

[ ] `TxnContext` остаётся единственным owner-ом runtime mutable state и COW
    semantics.
[ ] В коде не появляется второй runtime cache или новый derived snapshot.
[ ] Tracking cloned layers больше не требует шумного index-shift bookkeeping.
[ ] Base-node resolve использует один locator-based path вместо дублированных
    background/content веток.
[ ] Для `TxnContext` watchpoint зоны шага `8` не остаётся giant helper-ов,
