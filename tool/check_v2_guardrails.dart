...
Never _fail(_Violation violation) {
  stderr.writeln('FAIL: v2 guardrails');
  stderr.writeln('- $violation');
  exit(1);
}

void _checkV2PublicImports({
  required Directory root,
  required String rootAbsPosix,
  required String packageName,
}) {
  final publicDir = Directory(
    '${root.path}${Platform.pathSeparator}lib${Platform.pathSeparator}src${Platform.pathSeparator}v2${Platform.pathSeparator}public',
  );
        ...
        if (isDisallowed) {
          _fail(
            _Violation(
              ...
              line: lineNo,
              message:
                  'v2/public must not import/export input/render/view/serialization internals ($resolvedRepoRelPosix)',
            ),
          );
        }
      }
...
void _checkV2ControllerGuardrails({
  required Directory root,
  required String rootAbsPosix,
}) {
  final controllerDir = Directory(
    '${root.path}${Platform.pathSeparator}lib${Platform.pathSeparator}src${Platform.pathSeparator}v2${Platform.pathSeparator}controller',
  );
  ...
  if (!hasControllerEpoch) {
    _fail(
      _Violation(
        filePath: '/lib/src/v2/controller',
        line: 1,
        message:
            'controllerEpoch symbol is required for v2 epoch invalidation guardrails',
      ),
    );
  }
}
...
void main(List<String> args) {
  ...
  _checkV2PublicImports(
    root: root,
    rootAbsPosix: rootAbsPosix,
    packageName: packageName,
  );
  _checkV2ControllerGuardrails(root: root, rootAbsPosix: rootAbsPosix);

  stdout.writeln('OK: v2 guardrails');
}
