...
8 P1 entries from `iwb_canvas_engine_12_stage_reports.md` remain documented here as confirmed against the codebase state captured by this research note. Fixed entries were removed from this remaining-problems document.
...
The 8 remaining P1 entries collapse into 7 unique problem groups when overlapping symptoms are grouped by owning code path. The original report IDs stay listed for traceability.
...
| 2 | `CODEC-001` | Runtime schema load validation | Aggregate metadata budget is enforced by public DTO projection but not by runtime schema import before store install. |
| 3 | `INTERACTION-001` | Selection terminal admission | Marquee/point selection can commit from unreliable query facts because terminal admission does not gate on query status/skipped candidates. |
| 4 | `INTERACTION-002` | Context target admission | Context action target admission can use partial candidate results with skipped/unresolved handles. |
| 5 | `GEOMETRY-001` | Geometry inverse transform computation | Hit testing calls public-validating inverse construction, so a valid transform can throw on derived inverse coordinates. |
| 6 | `FRAME-001` | Overlay frame capture | Overlay capture uses a full main-frame snapshot instead of minimal overlay facts. |
| 7 | `TEST-001` | Release benchmark baseline gate | Release benchmark diff runs against an approved baseline file whose current status is `unapproved`, and diff fails closed. |
...
### 2. Runtime Lifecycle And Interaction Flow
...
### 3. Geometry, Spatial, And Frame Capture
...
### 4. Surface Integration And Release Readiness
