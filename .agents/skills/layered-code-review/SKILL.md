...
2. After Layer 1 is complete, read [Layer 2 — Proof and Anti-Slop Review](references/layer-2-proof-and-slop-review.md) for every unit contract review and final committed-range review, even when the reviewed diff does not change tests or another proof artifact. For other review modes, read it when the reviewed work adds or modifies tests, guardrails, analyzers, docs, plans, design artifacts, verification claims, proof fixtures, generated truth, source-of-truth artifacts, completion or readiness claims, migration proof, compatibility proof, structural enforcement, negative proof, or any artifact that can create false confidence.
...
Whenever Layer 2 runs, apply `Outcome-Proof Fit`, including its mandatory contract-scoped audit when Layer 2 was routed because the review is a unit or final review. Report only actionable code-review style findings for artifacts that create false confidence, duplicated truth, weak guardrails, or self-referential proof.
