...
description: Use when the user explicitly asks to create or update a repository research artifact in `docs/history/research/`.
---
...
4. Use multiple tasks only when the question naturally splits into
   independent areas, with a maximum of 10 total tasks.
...
- Use multiple parallel tasks only for independent investigation areas, with a
  maximum of 10 total tasks.
- Never create artificial investigation areas just to reach a minimum count.
- Never spawn more than 10 parallel tasks because of context overflow risk.
...
```text
docs/history/research/YYYY-MM-DD-topic-name.md
```

Create the directory if needed. Research is historical factual input from
creation, never a current decision owner. Use a short kebab-case topic name derived from
...
4. Use at most 10 tasks and at most 10 parallel tasks.
