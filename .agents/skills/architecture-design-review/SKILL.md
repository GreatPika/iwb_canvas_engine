...
description: Review a .design/ architecture design artifact before Change Contract authoring. Use when checking whether the selected design form, evidence, alternatives, future-pressure analysis, diagram assessment, source-of-truth impact, and contract handoff are strong enough to proceed. Return a chat verdict only; do not edit files.
---
...
- future step traceability metadata.

Those belong to `change-contract` and `change-contract-check`.
...
5. The artifact omits required sections from the paired design artifact template,
   keeps template placeholders as final content, or changes the fixed hard-gate
   or diagram-assessment rows in a way that prevents review.
...
9. The artifact uses `NEEDS_RESEARCH` or `ARCHITECTURE_GATE`; the design may be
   a useful gate artifact, but it is not contract-ready.
...
14. Owner, owning layer, seam, boundary, dependency direction, state/data
   ownership, or verification strategy is unresolved.
15. The selected form patches a downstream call site when the weakness belongs
   to a shared owner, invariant, contract, or boundary.
16. The selected form puts validation, normalization, policy, or compatibility
    handling away from the owning boundary without explicit evidence and trade-off.
17. The selected form violates repository dependency/import direction, layer
    ownership, or documented source-of-truth ownership.
18. The design creates a second source of truth without an invariant and proof
   strategy.
19. A shared-seam change lacks successor or retired seam, consumer order,
    retirement gate, or negative proof strategy.
20. A public API change lacks compatibility, migration, or contract-owner
    reasoning.
21. A bug-fix design lacks root-cause owner and reproducer strategy.
22. A refactor design lacks behavior-preservation or characterization strategy.
23. An analyzer or guardrail design lacks positive and negative recognition
    forms or bypass/false-positive risk reasoning.
24. Future pressure is likely from cited evidence but the artifact does not
    assess it or route to `NEEDS_RESEARCH` / `ARCHITECTURE_GATE`.
25. The selected form ignores a known future pressure in a way that will
...
30. A required test, analyzer, guardrail, docs check, semantic search, or other
...
31. The Change Contract handoff contains or requires slices, proof IDs, final
    gates, or contract text.
32. The recommended form is one of multiple materially different viable options,
    but the choice depends on product preference and no user decision is routed.
33. Materially different candidate forms were possible, but the artifact neither
...
4. Check that the artifact follows the paired template and records truthful
   research inputs or explicit absence
   of research inputs.
